...
- “CLI 并发”限制同时运行的 Codex 进程数，默认 4，避免同时启动过多进程占用内存。
...
默认请求次数为 5，重试次数为 5，因此最多运行 6 批。直接 API 模式的默认上限是 30 个 POST；官方 CLI 模式的默认上限是 30 个逻辑任务。任意任务成功后不会再开启下一批，已经启动的同批任务会继续收尾。
